...
/**
 * @def_driverbackendgroup{RFID Interface,rfid_interface}
 * @{
 */
...
/**
 * @driver_ops{RFID Interface}
 */
__subsystem struct rfid_driver_api {
	/**
	 * @driver_ops_mandatory @copybrief rfid_claim
	 */
	rfid_claim_t claim;
	/**
	 * @driver_ops_mandatory @copybrief rfid_release
	 */
	rfid_release_t release;
	/**
	 * @driver_ops_mandatory @copybrief rfid_load_protocol
	 */
	rfid_load_protocol_t load_protocol;
	/**
	 * @driver_ops_optional @copybrief rfid_get_properties
	 */
	rfid_properties_get_t get_properties;
	/**
	 * @driver_ops_optional @copybrief rfid_set_properties
	 */
	rfid_properties_set_t set_properties;
	/**
	 * @driver_ops_optional @copybrief rfid_initiator_transceive
	 */
	rfid_initiator_transceive_t im_transceive;
	/**
	 * @driver_ops_optional @copybrief rfid_listen
	 */
	rfid_listen_t listen;
	/**
	 * @driver_ops_optional @copybrief rfid_target_transmit
	 */
	rfid_target_transmit_t tm_transmit;
	/**
	 * @driver_ops_optional @copybrief rfid_target_receive
	 */
	rfid_target_receive_t tm_receive;
	/**
	 * @driver_ops_mandatory @copybrief rfid_supported_protocols
	 */
	rfid_supported_protocols_t supported_protocols;
	/**
	 * @driver_ops_mandatory @copybrief rfid_supported_modes
	 */
...
{
	return DEVICE_API_GET(rfid, dev)->claim(dev);
}
...
{
	return DEVICE_API_GET(rfid, dev)->release(dev);
}
...
static inline int z_impl_rfid_load_protocol(const struct device *dev, rfid_proto_t proto,
					    rfid_mode_t mode)
{
	if (DEVICE_API_GET(rfid, dev)->load_protocol == NULL) {
		return -ENOSYS;
	}

	return DEVICE_API_GET(rfid, dev)->load_protocol(dev, proto, mode);
}
...
static inline int z_impl_rfid_get_properties(const struct device *dev, struct rfid_property *props,
					     size_t props_len)
{
	if (DEVICE_API_GET(rfid, dev)->get_properties == NULL) {
		return -ENOSYS;
	}

	return DEVICE_API_GET(rfid, dev)->get_properties(dev, props, props_len);
}
...
static inline int z_impl_rfid_set_properties(const struct device *dev, struct rfid_property *props,
					     size_t props_len)
{
	if (DEVICE_API_GET(rfid, dev)->set_properties == NULL) {
		return -ENOSYS;
	}

	return DEVICE_API_GET(rfid, dev)->set_properties(dev, props, props_len);
}
...
{
	if (DEVICE_API_GET(rfid, dev)->im_transceive == NULL) {
		return -ENOSYS;
	}

	return DEVICE_API_GET(rfid, dev)->im_transceive(dev, tx_data, tx_len, tx_last_bits, rx_data,
									rx_len);
}
...
static inline int z_impl_rfid_target_transmit(const struct device *dev, const uint8_t *tx_data,
					      uint16_t tx_len, uint8_t tx_last_bits)
{
	if (DEVICE_API_GET(rfid, dev)->tm_transmit == NULL) {
		return -ENOSYS;
	}

	return DEVICE_API_GET(rfid, dev)->tm_transmit(dev, tx_data, tx_len, tx_last_bits);
}
...
static inline int z_impl_rfid_target_receive(const struct device *dev, uint8_t *rx_data,
					     uint16_t *rx_len)
{
	if (DEVICE_API_GET(rfid, dev)->tm_receive == NULL) {
		return -ENOSYS;
	}

	return DEVICE_API_GET(rfid, dev)->tm_receive(dev, rx_data, rx_len);
}
...
static inline int z_impl_rfid_listen(const struct device *dev, rfid_proto_t proto, uint8_t *rx_data,
				     uint16_t *rx_len)
{
	if (DEVICE_API_GET(rfid, dev)->listen == NULL) {
		return -ENOSYS;
	}

	return DEVICE_API_GET(rfid, dev)->listen(dev, proto, rx_data, rx_len);
}
...
{
	if (DEVICE_API_GET(rfid, dev)->supported_protocols == NULL) {
		return -ENOSYS;
	}

	return DEVICE_API_GET(rfid, dev)->supported_protocols(dev);
}
...
{
	if (DEVICE_API_GET(rfid, dev)->supported_modes == NULL) {
		return -ENOSYS;
	}

	return DEVICE_API_GET(rfid, dev)->supported_modes(dev, proto);
}
